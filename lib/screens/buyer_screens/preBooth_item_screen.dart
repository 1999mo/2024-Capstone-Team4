...
class PreboothItemScreen extends StatefulWidget {
  final String uid;
  final String? festivalName;
  final String itemName;

  const PreboothItemScreen({
    Key? key,
    required this.uid,
  ...
  @override
  _PreboothItemScreenState createState() => _PreboothItemScreenState();
}

class _PreboothItemScreenState extends State<PreboothItemScreen> {
  int itemQuantity = 1;
                    ...
                    onPressed: () async {
                      try {
                        ...
                        DocumentSnapshot doc = await FirebaseFirestore.instance
                            ...
                            .doc(user?.uid)
                            .collection('basket')
                            .doc(widget.festivalName).get();

                        if(doc.exists) {
                          Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
                            if(data.containsKey(widget.uid)) {
                              Map<String, dynamic> boothData = data[widget.uid] as Map<String, dynamic>;

                              if (boothData.containsKey('payTrue')) {
                                if(boothData['payment'] == true)
                                  return;
                              }
                          }
                        }
                        ...
                        await FirebaseFirestore.instance
                            ...
                            .doc(user?.uid)
                            .collection('basket')
                            .doc(widget.festivalName).set({
                          widget.uid : {
                              widget.itemName : itemQuantity,
                              'payment' : false,
                          },
