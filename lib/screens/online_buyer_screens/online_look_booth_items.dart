...
class _OnlineLookBoothItemsState extends State<OnlineLookBoothItems> {
  ...
  Future<String?> _getImageUrl(String itemId) async {
    try {
      ...
      final ref = FirebaseStorage.instance.ref('$sellerUid/$processedItemId.jpg');
  ...
  Widget build(BuildContext context) {
    if (festivalName == null || sellerUid == null) {
      return Scaffold(
        appBar: AppBar(title: const Text('상품 둘러보기')),
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream:
            FirebaseFirestore.instance.collection('OnlineStore').doc(festivalName).collection(sellerUid!).snapshots(),
                                    ...
                                    content: SingleChildScrollView(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text('상품명: ${itemData['itemName'] ?? 'N/A'}'),
                                          Text('작가명: ${itemData['artist'] ?? 'N/A'}'),
                                          Text('상품 종류: ${itemData['itemType'] ?? 'N/A'}'),
                                          Text('판매가: ${itemData['sellingPrice'] ?? 'N/A'}원'),
                                      ...
                                      GestureDetector(
                                        onTap: () async {
                                          ...
                                          try {
                                            ...
                                            ScaffoldMessenger.of(context).showSnackBar(
                                              const SnackBar(content: Text('상품이 장바구니에 추가되었습니다.'), duration: Duration(seconds: 1),),
                                            );
                                            ...
                                            final basketRef = FirebaseFirestore.instance
                                                ...
                                                .doc(festivalName); // 축제 이름에 해당하는 문서
                                            ...
                                            final newItem = {
                                              ...
                                              'imagePath': '${sellerUid}/${itemId.replaceAll(' ', '_')}.jpg',
                                            };
                                            ...
                                            if (snapshot.exists) {
                                              ...
                                              if (currentData.containsKey(sellerUid)) {
                                                // 해당 sellerId 필드가 존재하면 기존 배열에서 itemName 비교
                                                final List<dynamic> currentItems =
                                                    currentData[sellerUid] as List<dynamic>;

                                                // 동일한 itemName을 가진 항목이 있는지 확인
                                                currentItems
                                                    .removeWhere((item) => item['itemName'] == itemData['itemName']);
                                            ...
                                            ScaffoldMessenger.of(context).showSnackBar(
                                              SnackBar(content: Text('장바구니 추가 중 오류가 발생했습니다: $e')),
                                            );
                                          }
                                        },
                                        child: Container(
